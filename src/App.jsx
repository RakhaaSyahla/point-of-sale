...
export default function App() {
  const initialProducts = useMemo(() => ([
    { id: 1, name: 'Makanan', description: 'Produk makanan siap saji', price: 10000, category: 'Makanan', releaseDate: '2024-01-01', stock: 10, active: true },
    { id: 2, name: 'Minuman', description: 'Aneka minuman dingin & hangat', price: 8000, category: 'Minuman', releaseDate: '2024-02-01', stock: 20, active: true },
  ]), [])
  ...
  const validate = () => {
    ...
    if (trimmedDesc) {
      if (trimmedDesc.length < 20) newErrors.description = 'Deskripsi minimal 20 karakter.'
      if (trimmedDesc.length > 1000) newErrors.description = 'Deskripsi terlalu panjang.'
    }

    // price
    if (String(price).trim() === '') newErrors.price = 'Harga wajib diisi.'
    else if (Number.isNaN(parsedPrice)) newErrors.price = 'Harga harus numerik.'
    ...
    // release date
    if (releaseDate) {
      if (!(rd instanceof Date) || Number.isNaN(rd.getTime())) newErrors.releaseDate = 'Format tanggal tidak valid.'
      else if (rd > today) newErrors.releaseDate = 'Tanggal rilis tidak boleh di masa depan.'
    }
  ...
  const categoryOptions = ['Elektronik', 'Pakaian', 'Makanan', 'Minuman', 'Rumah Tangga', 'Lainnya']
              ...
              <Table striped bordered hover responsive className="mb-0">
                <thead>
                  <tr>
                    ...
                    <th style={{ width: 200 }} className="text-center">Aksi</th>
                  </tr>
                </thead>
                <tbody>
                  {categories.length === 0 ? (
                      ...
                      <tr key={cat.id}>
                        ...
                        <td className="text-center">
                          <div className="d-flex justify-content-center gap-2">
                            <Button size="sm" variant="warning" onClick={() => handleEdit(cat)}>Edit</Button>
                            <Button size="sm" variant="danger" onClick={() => handleDelete(cat.id)}>Hapus</Button>
                          </div>
                        </td>
                      </tr>
                    ))
